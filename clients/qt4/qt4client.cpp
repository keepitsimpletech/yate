...
static void qtMsgHandler(QtMsgType type, const QMessageLogContext &context, const QString &text)
{
    ...
    Debug("QT",dbg,"%s",text.toUtf8().constData());
}
...
QtWindow::~QtWindow()
{
    ...
    QList<QSystemTrayIcon*> trayIcons = this->findChildren<QSystemTrayIcon*>();
    if (trayIcons.size() > 0) {
	...
	QList<QSplitter*> spl = this->findChildren<QSplitter*>();
	...
	QList<QObject*> child = this->findChildren<QObject*>();
...
bool QtWindow::buildMenu(const NamedList& params)
{
    ...
    if (owner && owner != m_id) {
	parent = this->findChild<QWidget*>(QtClient::setUtf8(owner));
	if (!parent) {
    ...
    if (t) {
	target = this->findChild<QWidget*>(QtClient::setUtf8(t));
	if (!target) {
	    DDebug(QtDriver::self(),DebugNote,
...
bool QtWindow::setImage(const String& name, const String& image, bool fit)
{
    ...
    QObject* obj = this->findChild<QObject*>(QtClient::setUtf8(name));
...
void QtWindow::keyPressEvent(QKeyEvent* event)
{
	...
	QWidget* wid = this->findChild<QWidget*>(child);
	if (wid)
	    wid->setFocus();
	return;
    }
...
void QtWindow::setVisible(bool visible)
{
    ...
    if (!m_visible) {
	QList<QDialog*> d = this->findChildren<QDialog*>();
...
{
    QDialog* d = this->findChild<QDialog*>(QtClient::setUtf8(name));
    if (!d)
	return false;
    d->deleteLater();
    return true;
...
void QtWindow::doInit()
{
    ...
    QList<QFrame*> frm = this->findChildren<QFrame*>();
	...
	if (sect && *sect && *sect != "general")
	    addDynamicProps(this->findChild<QObject*>(sect->c_str()),*sect);
    }
    ...
    QList<QGroupBox*> grp = this->findChildren<QGroupBox*>();
    ...
    QList<QCalendarWidget*> cals = this->findChildren<QCalendarWidget*>();
	...
	if (!QtClient::getBoolProperty(tables[i],"_yate_verticalheader"))
		...
		hdr->sectionResizeMode(QHeaderView::Fixed);
	}
	if (nonCustom) {
    ...
    QList<QObject*> w = this->findChildren<QObject*>();
    w.append(this);
...
bool QtWindow::handleContextMenuEvent(QContextMenuEvent* event, QObject* obj)
{
    ...
    QMenu* m = mname ? this->findChild<QMenu*>(QtClient::setUtf8(mname)) : 0;
    if (m)
...
{
    if (!(!QAudioDeviceInfo::availableDevices(QAudio::AudioOutput).isEmpty() && name && *name && file && *file))
	return false;
...
QUrl QtUrlBuilder::build(const NamedList& params) const
{
	...
	for (const NamedString* ns = 0; 0 != (ns = iter.get());)
		...
		query.addQueryItem(QtClient::setUtf8(ns->name()),QtClient::setUtf8(*ns));
	url.setQuery(query);
    }
    return url;
}
